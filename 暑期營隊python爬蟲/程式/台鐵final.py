...
end_station = end_station_list[int(input('請輸入抵達站代碼: '))]
print()

print(f'請輸入日期,包含西元年月日,只有從今日({timenow})起90天內的日期為有效日期')
year = int(input('年: '))
month = int(input('月: '))
day = int(input('日: '))
print()
...
print(f'以下是{year}/{month}/{day} {start_station[5:]}到{end_station[5:]}的班次表\n')
print(final_data.to_string(index=False))

final_data.to_csv(f'D:/vs code file/web scraper/暑期營隊python爬蟲/台鐵班次表/{year}-{month}-{day} {start_station[5:]}到{end_station[5:]}.csv',index = False, encoding = 'utf-8-sig')
